...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------
def plot_farness_centrality(CG, IG, H_IG, pos):
    fc = {}
    T = 0
    ...
    for i in H_IG:
        ...
        T_H = (sum(list(current.values())) / (diame * (num_nod - 1)))*100
        T = T + T_H
    ...
    newcmp = ListedColormap(hsv_modified(np.linspace(0.75, 0.95, 256)))  # show figure
    cmap = newcmp
    ...
    ax = plt.gca()
    ax.set_axis_off()
    ax.set_title(sum(list(fc.values())))
    nc = nx.draw_networkx_nodes(CG, pos, nodelist=list(fc.keys()), node_color=list(fc.values()), node_size=3, cmap=cmap)
    plt.colorbar(nc)
    #plt.xlim(-5.7, 3.8)
    #plt.ylim(-1.8, 3.9)
    plt.axis("off")
    plt.title('Global Unsafely Farness = ' + str(T) + '%')
    plt.savefig('farness_centrality.eps', format='eps')
    plt.show()
#-------------------------------------------------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------------------------------------------------
...
CG = current_graph(G)
IG = inverse_weight(G)
connected_c_IG = sorted(nx.connected_components(CG), key=len)
H_IG = []
for i in connected_c_IG:
    H_IG.append(CG.subgraph(i))
plot_farness_centrality(G, IG, H_IG, pos)
